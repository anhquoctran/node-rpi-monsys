...
function processes(callback) {
    ...
    function parseLine(line) {
        var pid = parseInt(line.substring(parsedhead[0].from, parsedhead[0].to));
        var pcpu = parseFloat(line.substring(parsedhead[1].from, parsedhead[1].to).replace(/,/g, "."));
        var pmem = parseFloat(line.substring(parsedhead[2].from, parsedhead[2].to).replace(/,/g, "."));
        var priority = parseInt(line.substring(parsedhead[3].from, parsedhead[3].to));
        var vsz = parseInt(line.substring(parsedhead[4].from, parsedhead[4].to));
        var rss = parseInt(line.substring(parsedhead[5].from, parsedhead[5].to));
        var nice = parseInt(line.substring(parsedhead[6].from, parsedhead[6].to));
        var started = line.substring(parsedhead[7].from, parsedhead[7].to).trim();
        var state = line.substring(parsedhead[8].from, parsedhead[8].to).trim();
        state = (state[0] == 'R' ? 'running' : (state[0] == 'S' ? 'sleeping' : (state[0] == 'T' ? 'stopped' : (state[0] == 'W' ? 'paging' : (state[0] == 'X' ? 'dead' : (state[0] == 'Z' ? 'zombie' : ((state[0] == 'D' || state[0] == 'U') ? 'blocked' : 'unknown')))))));
        var tty = line.substring(parsedhead[9].from, parsedhead[9].to).trim();
        if (tty == '?' || tty == '??') tty = '';
        var user = line.substring(parsedhead[10].from, parsedhead[10].to).trim();
        var command = line.substring(parsedhead[11].from, parsedhead[11].to).trim().replace(/\[/g, "").replace(/]/g, "");
                    ...
                    if (!error) {
                        ...
                        if (_linux) {
                            ...
                            exec(cmd, function(error, stdout) {
                                ...
                                var all = parseProcStat(curr_processes.shift());

                                // process
                                var list_new = {};
                                var resultProcess = {};
                                for (var i = 0; i < curr_processes.length; i++) {
                                    resultProcess = parseProcPidStat(curr_processes[i], all);

                                    if (resultProcess.pid) {

                                        // store pcpu in outer array
                                        var listPos = result.list.map(function(e) { return e.pid; }).indexOf(resultProcess.pid);
                                        if (listPos >= 0) {
