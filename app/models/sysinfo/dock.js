...
const DockerSocket = require('./dockSocket')
...
function dockerContainers(all, callback) {
    ...
    function inContainers(containers, id) {
        var filtered = containers.filter(obj => {
            /**
             * @namespace
             * @property {string}  Id
             */
        ...
        process.nextTick(() => {
            ...
            _docker_socket.listContainers(all, data => {
                var docker_containers = {}
                    // var cmd = "curl --unix-socket /var/run/docker.sock http:/containers/json" + (all ? "?all=1" : "")
                    // exec(cmd, function (error, stdout) {
                    //   if (!error) {
                try {
                    //       var jsonString = stdout.toString()
        ...
        process.nextTick(() => {
            ...
            if (containerID) {
                ...
                _docker_socket.getStats(containerID, data => {
                    // var cmd = "curl --unix-socket /var/run/docker.sock http:/containers/" + containerID + "/stats?stream=0"
                    // exec(cmd, function (error, stdout) {
                    //   if (!error) {
                    //     var jsonString = stdout.toString()
                    try {
